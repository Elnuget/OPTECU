...
<div class="card">
    <div class="card-body">
        <form id="formEditarSueldo" action="{{ route('sueldos.update', $sueldo->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-row">
                <div class="form-group col-md-6">
                    ...
                    <select name="user_id" id="user_id" class="form-control select2" required>
                        ...
                        @foreach(\App\Models\User::orderBy('name')->get() as $user)
                            <option value="{{ $user->id }}" {{ $sueldo->user_id == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-6">
                    ...
                    <select name="empresa_id" id="empresa_id" class="form-control select2">
                        ...
                        @foreach(\App\Models\Empresa::orderBy('nombre')->get() as $empresa)
                            <option value="{{ $empresa->id }}" {{ $sueldo->empresa_id == $empresa->id ? 'selected' : '' }}>
                                {{ $empresa->nombre }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="form-row">
                <div class="form-group col-md-6">
                    ...
                    <input type="date" class="form-control" id="fecha" name="fecha" 
                        value="{{ $sueldo->fecha->format('Y-m-d') }}" required>
                </div>
                <div class="form-group col-md-6">
                    <label for="valor">Valor ($):</label>
                    <input type="number" step="0.01" class="form-control" id="valor" 
                        name="valor" value="{{ $sueldo->valor }}" required>
                </div>
            </div>
            <div class="form-group">
                <label for="descripcion">Descripción:</label>
                <textarea class="form-control" id="descripcion" name="descripcion" rows="3" required>{{ $sueldo->descripcion }}</textarea>
            </div>
    ...
    $('#formEditarSueldo').submit(function(e) {
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ...
                let errorMessage = 'Error al actualizar el sueldo';
